...
- **Next.js 16 App Router** — 6 个页面路由: `/`(大厅) `/market`(虫市) `/backpack`(背包) `/matchmake`(自动匹配) `/room/[roomId]`(房间) `/battle/[roomId]`(战斗)。当前所有页面均为 `use client`
...
- **游戏配置集中化** — 所有魔法数字在 `src/config/game.ts`，包括伤害参数、`DAMAGE_MULTIPLIER`(伤害倍率)、`MIN_CRICKETS_TO_BATTLE`(出战蛐蛐数)、克制倍率、耐力消耗、斗性/品质/特性常量
...
src/
  app/                         # Next.js 页面 & API routes
    ...
    matchmake/page.tsx         # 自动匹配: WS 匹配+超时+重试
...
| C→S | room:matchmake | 自动匹配 (传 uid, nickName) |
| C→S | room:matchmake.cancel | 取消匹配 (传 uid) |
...
| S→C | room:matched | 匹配成功 (含 roomId) |
| S→C | room:matchmake.timeout | 匹配超时 |
