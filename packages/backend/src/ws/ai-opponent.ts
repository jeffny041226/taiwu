...
const COUNTER_WEIGHTS: Record<Action, { heavy_strike: number; feint: number; block: number; chirp: number }> = {
  heavy_strike: { heavy_strike: 10, feint: 15, block: 55, chirp: 20 },
  feint:        { heavy_strike: 50, feint: 10, block: 15, chirp: 25 },
  block:        { heavy_strike: 25, feint: 45, block: 10, chirp: 20 },
  chirp:        { heavy_strike: 50, feint: 20, block: 15, chirp: 15 },
};
...
export function aiChooseAction(
  aiCricket: BattleCalcInput,
  playerLastAction: Action | null,
  playerCricket: BattleCalcInput
): Action {
  if (!playerLastAction) {
...
function weightedRandom(weights: Record<Action, number>): Action {
...
export function createAIPlayer(ws?: WebSocket) {
  return {
    ...
    readyRound: true,
    ws: ws as WebSocket | null,
  };
}

export function createAICrickets(): BattleCricket[] {
  return [
    { id: 1001, templateId: 7, name: "青头大王", title: "青面獠牙", tier: "rare", attack: 16, defense: 18, speed: 14, maxHp: 110, hp: 110, maxStamina: 105, stamina: 105, spirit: 105, trait: "resonant" },
    { id: 1002, templateId: 8, name: "黑头金刚", title: "黑甲战神", tier: "rare", attack: 18, defense: 16, speed: 15, maxHp: 110, hp: 110, maxStamina: 105, stamina: 105, spirit: 105, trait: "fierce" },
    { id: 1003, templateId: 10, name: "铁翅元帅", title: "铁翼横空", tier: "rare", attack: 17, defense: 22, speed: 12, maxHp: 110, hp: 110, maxStamina: 105, stamina: 105, spirit: 105, trait: "steadfast" },
  ];
}